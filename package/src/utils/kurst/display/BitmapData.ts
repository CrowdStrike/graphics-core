...
export class BitmapData {
   ...
   */
  clone() {
    let t = new BitmapData(this.width, this.height, this.transparent);
   ...
   * </ul>
   *
   * @param sourceBitmap The input bitmap image to use. The source image
  ...
  copyChannel(
    ...
    let sourceData = sourceBitmap.imageData.data;
    let destData = this._imageData.data;
    let sourceOffset = Math.round(Math.log(sourceChannel) / Math.log(2));
    let destOffset = Math.round(Math.log(destChannel) / Math.log(2));
    let i;
    let j;
    let sourceIndex;
    let destIndex;
  ...
  private _copyPixels(
    ...
    if (bmpd instanceof BitmapData && this._context) {
   ...
   * @private
   *
   * NOTE: Smoothing needs to be implemented at somepoint
   *
   */
  private _draw(
    source: BitmapData | HTMLImageElement | HTMLCanvasElement,
    matrix?: Matrix,
    clipRect?: Rectangle /* , smoothing */,
  ) {
    if (source instanceof BitmapData && this._context) {
      this._context.save();
  ...
  getPixel(x: number, y: number) {
    ...
    if (!this._locked && this._context) {
      let pixelData = this._context.getImageData(x, y, 1, 1);
      ...
      return (r << 16) | (g << 8) | b;
    } else if (this._imageCanvas && this._imageData) {
