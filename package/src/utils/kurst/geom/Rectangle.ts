...
export class Rectangle {
  ...
  private _bottomRight: Point = new Point();
  private _topLeft: Point = new Point();
   ...
   *  without parameters, a rectangle with <code>x</code>,
  ...
  intersection(toIntersect: Rectangle): Rectangle {
    if (this.intersects(toIntersect)) {
      let i: Rectangle = new Rectangle();

      if (this.x > toIntersect.x) {
        i.x = this.x;
        i.width = toIntersect.x - this.x + toIntersect.width;

        if (i.width > this.width) i.width = this.width;
      } else {
        i.x = toIntersect.x;
        i.width = this.x - toIntersect.x + this.width;

        if (i.width > toIntersect.width) i.width = toIntersect.width;
      }
      ...
      if (this.y > toIntersect.y) {
        ...
        if (i.height > this.height) i.height = this.height;
      } else {
        i.y = toIntersect.y;
        i.height = this.y - toIntersect.y + this.height;

        if (i.height > toIntersect.height) i.height = toIntersect.height;
      }
  ...
  union(toUnion: Rectangle): Rectangle {
    ...
    if (this.x < toUnion.x) {
      ...
      if (u.width < toUnion.width) u.width = toUnion.width;
    }
    ...
    if (this.y < toUnion.y) {
      ...
      if (u.height < toUnion.height) u.height = toUnion.height;
    }
